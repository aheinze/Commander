...
pub struct Screen {
    ...
    // Only consecutive printed scalars can extend the preceding grapheme.
    last_printed: Option<(crate::grid::Pos, crate::grid::Pos)>,
}
...
impl Screen {
    ...
    fn extend_grapheme(&mut self, c: char) -> bool {
        let Some((start, cursor)) = self.last_printed else {
            return false;
        };
        if self.grid().pos() != cursor {
            return false;
        }
        let Some(cell) = self.grid().drawing_cell(start) else {
            return false;
        };
        // Preserve VT's scalar-cell behavior for other spacing characters
        // (for example Arabic prepend characters). Only emoji introduce new
        // spacing scalars inside a terminal cell.
        if c.width().unwrap_or(1) > 0
            && !matches!(c, '\u{1f1e6}'..='\u{1f1ff}' | '\u{1f3fb}'..='\u{1f3ff}')
            && !cell.contents().ends_with('\u{200d}')
        {
            return false;
        }
        let mut text = cell.contents().to_owned();
        text.push(c);
        if text.graphemes(true).nth(1).is_some() {
            return false;
        }
        // Bound malicious runs of combining marks without splitting valid emoji.
        if text.len() > crate::cell::CONTENT_BYTES {
            return true;
        }
        let old_width = if cell.is_wide() { 2 } else { 1 };
        let width = (text.width() as u16)
            .clamp(1, 2)
            .min(self.grid().size().cols);
        let attrs = *cell.attrs();
        if width == old_width {
            self.grid_mut()
                .drawing_cell_mut(start)
                .unwrap()
                .set_grapheme(&text, width, attrs);
        } else {
            // Reuse the normal overwrite/wrap path when VS16 or a flag grows
            // a one-column character into a two-column grapheme.
            self.grid_mut().col_dec(old_width);
            let wraps = start.col + width > self.grid().size().cols;
            if wraps {
                // Remove the provisional narrow glyph before wrapping, even
                // when this row is about to move into scrollback. This gives
                // the cluster the same wrap semantics as a wide scalar.
                self.grid_mut()
                    .drawing_cell_mut(start)
                    .unwrap()
                    .clear(attrs);
            }
            self.draw_grapheme(&text, width);
        }
        true
    }

    fn draw_grapheme(&mut self, text: &str, width: u16) {
        let c = text.chars().next().unwrap();
        let pos = self.grid().pos();
        let size = self.grid().size();
        let attrs = self.attrs;

        if c.width().is_none() && (u32::from(c)) < 256 {
        ...
        if width == 0 {
            ...
            let cell = self
                ...
                // that self.grid().pos().col has a valid value.
                .unwrap();
            cell.set_grapheme(text, width, attrs);
            ...
            self.last_printed = Some((pos, self.grid().pos()));
        }
    }

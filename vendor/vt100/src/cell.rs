...
// Bounded storage large enough for emoji families, modifiers and tag sequences.
pub(crate) const CONTENT_BYTES: usize = 54;

const IS_WIDE: u8 = 0b1000_0000;
const IS_WIDE_CONTINUATION: u8 = 0b0100_0000;
const LEN_BITS: u8 = 0b0011_1111;
...
const _: () = assert!(std::mem::size_of::<Cell>() == 64);
...
impl Cell {
    ...
    pub(crate) fn set_grapheme(
        &mut self,
        text: &str,
        width: u16,
        attrs: crate::attrs::Attrs,
    ) {
        assert!(text.len() <= CONTENT_BYTES);
        self.contents[..text.len()].copy_from_slice(text.as_bytes());
        self.len = u8::try_from(text.len()).unwrap();
    ...
    ///
    /// Contains a complete grapheme, including emoji joiners and modifiers.

...
pub(super) fn sync_from_compare(
    vfs: &dyn Vfs,
    engine: OperationEngine,
    left: &VPath,
    right: &VPath,
    ...
    let (source_root, destination_root) = match direction {
    ...
    for entry in entries {
        cancel
            .check()
            .map_err(|_| "Folder synchronization cancelled".to_owned())?;
        let should_copy = match direction {
            SyncDirection::LeftToRight => matches!(
                entry.status,
                CompareStatus::LeftOnly | CompareStatus::Different
            ),
            SyncDirection::RightToLeft => matches!(
                entry.status,
                CompareStatus::RightOnly | CompareStatus::Different
            ),
        };
        if !should_copy {
            continue;
        ...
        let metadata = match direction {
            SyncDirection::LeftToRight => entry.left.as_ref(),
            SyncDirection::RightToLeft => entry.right.as_ref(),
        }
        .ok_or_else(|| {
            format!(
        ...
        let source = VPath::from(source_root.as_path().join(&entry.relative_path));
        let destination = VPath::from(destination_root.as_path().join(&entry.relative_path));
        if metadata.kind == EntryKind::Directory {
            match vfs.stat(&destination, false) {
                Ok(existing) if existing.kind == EntryKind::Directory => continue,
                Ok(_) => trash_sync_target(&engine, destination.clone())?,
                Err(_) => {}
            }
        ...
        if vfs.stat(&destination, false).is_ok() {
            trash_sync_target(&engine, destination.clone())?;
        }
        match metadata.kind {
            EntryKind::File => {
                let mut reader = vfs
                ...
                let mut writer = vfs
                    .create_write(&destination, SizeHint::Exact(metadata.size))
                    .map_err(|error| format!("Could not create {destination}: {error}"))?;
                ...
                loop {
                    ...
                    let count = reader.read(&mut buffer).map_err(|error| {
                        format!("Could not read {source} during synchronization: {error}")
                    })?;
                    if count == 0 {
                        break;
                    }
                    writer.write_all(&buffer[..count]).map_err(|error| {
                        format!("Could not write {destination} during synchronization: {error}")
                    })?;
                }
                writer
                    .flush()
                    .map_err(|error| format!("Could not flush {destination}: {error}"))?;
                drop(writer);
                let _ = vfs.preserve_metadata(&source, &destination, metadata);
            }
            EntryKind::Symlink => {
                let target = vfs
                    .read_link(&source)
                    .map_err(|error| format!("Could not read link {source}: {error}"))?;
                vfs.create_symlink(&VPath::from(target), &destination)
                    .map_err(|error| format!("Could not create link {destination}: {error}"))?;
            }
            kind => {
                return Err(format!(
                    "Synchronization does not support {kind:?} entry {source}"
                ));
            }
        }
        changed = changed.saturating_add(1);
    ...
    if mirror {
        let mut extras = entries
            ...
            .filter(|entry| match direction {
                SyncDirection::LeftToRight => entry.status == CompareStatus::RightOnly,
                SyncDirection::RightToLeft => entry.status == CompareStatus::LeftOnly,
            })
            .map(|entry| entry.relative_path.clone())
            .collect::<Vec<_>>();
        extras.sort_by_key(|path| path.components().count());
        let mut top_level = Vec::<std::path::PathBuf>::new();
        for path in extras {
            if !top_level.iter().any(|parent| path.starts_with(parent)) {
                top_level.push(path);
            }
        }
        if !top_level.is_empty() {
            ...
            let paths = top_level
                .iter()
                .map(|path| VPath::from(destination_root.as_path().join(path)))
                .collect::<Vec<_>>();
            let summary = engine.spawn_trash(paths).join();
            if summary.state != JobState::Done || !summary.outcome.errors.is_empty() {
                return Err(summary.outcome.errors.first().map_or_else(
                    || format!("Mirror cleanup ended in {:?}", summary.state),
                    |error| error.message.clone(),
                ));
            }
            changed = changed.saturating_add(top_level.len());
        }
    }
    Ok(changed)
}

pub(super) fn trash_sync_target(engine: &OperationEngine, path: VPath) -> Result<(), String> {
    let summary = engine.spawn_trash(vec![path]).join();
    if summary.state == JobState::Done && summary.outcome.errors.is_empty() {
        Ok(())
    } else {
        Err(summary.outcome.errors.first().map_or_else(
            || format!("Could not replace destination: {:?}", summary.state),
            |error| error.message.clone(),
        ))
    }
}
